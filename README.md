...
Both programs take settings as pairs `<Name> <file>` on the command line. `<Name>` becomes the prefix of every key in that file, so `Seed 42` in the file passed as `General` is read as `General/Seed`. File syntax: one `Key Value` per line, `#` starts a comment, blank and comment-only lines are skipped, and for a single-valued key anything after the second token is ignored. A comma-separated list uses the whole rest of the line and may have spaces after the commas, so `TrackToDraw 1, 2,3` is three tracks, but a list entry may not contain whitespace itself, so `TrackToDraw 1 2 3` is an error rather than the number 123. Booleans are the literal `true` or `false`. Errors that name the file, line or key: a key without a value, a value that the conversion does not consume in full (`2e4` or `0.37m` where a number is expected), a negative value where a count is expected, a boolean that is neither `true` nor `false`, and duplicate keys. Both programmes print a one-line message and exit with status 1 when the arguments do not come in `<Name> <file>` pairs, when the `RunARC` run mode is neither of the two below, when `General/BarrelOrEndcap` is neither `Barrel` nor `EndCap`, and when the `OptimiseARC` column and row are not non-negative integers. Six blocks are used; the committed files in `options/` are a working barrel setup.
...
- `CherenkovAngleResolution`: generates `NumberTracks` tracks, traces and reconstructs their photons and writes `CherenkovFile.root` containing `CherenkovTree`, one entry per track: momentum and direction, entrance and mirror-hit points, initial and final cell indices, per-photon true and reconstructed Cherenkov angles (with true and assumed emission point), energies, hit positions, migration flags and status codes, and the per-track single-photon resolution, total resolution and the separation significance between the two mass hypotheses. A track that never reaches a mirror, because it misses the entrance window or leaves the array, is still written as an entry with no photons, carrying -1 in the three resolution and significance branches, NaN in `MirrorHit_x/y/z` (since -1 m is a position a mirror hit could have) and the largest `std::size_t` in the final cell indices. It also writes an event display PDF of the row selected by `EventDisplay/RowToDraw` with the tracks listed in `General/TrackToDraw`, where the photon paths are coloured by status; paths of photons that reach the detector plane but miss the SiPM now get their own colour, before they were drawn in the colour of every other failure.
- `SingleTrack`: propagates one track defined by `Particle/Momentum`, `Particle/CosTheta`, `Particle/Phi` and draws its photon hits on the SiPM of the cell it reaches to `PhotonHits.pdf`. Only photons that are detected are plotted, and their number is printed.

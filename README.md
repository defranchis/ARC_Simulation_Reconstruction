...
Both programs take settings as pairs `<Name> <file>` on the command line. `<Name>` becomes the prefix of every key in that file, so `Seed 42` in the file passed as `General` is read as `General/Seed`. File syntax: one `Key Value` per line, `#` starts a comment, blank and comment-only lines are skipped, and for a single-valued key anything after the second token is ignored. A comma-separated list uses the whole rest of the line and may have spaces after the commas, so `TrackToDraw 1, 2,3` is three tracks, but a list entry may not contain whitespace itself, so `TrackToDraw 1 2 3` is an error. Booleans are the literal `true` or `false`. Errors that name the file, line or key: a key without a value, a value that the conversion does not consume in full (`0.37m` where a number is expected, `2e4` where an integer is expected), a negative value where a count is expected, a boolean that is neither `true` nor `false`, and duplicate keys. Both programmes print a one-line message and exit with status 1 when the arguments do not come in `<Name> <file>` pairs, when the `RunARC` run mode is neither of the two below, when `General/BarrelOrEndcap` is neither `Barrel` nor `EndCap`, and when the `OptimiseARC` column and row are not non-negative integers. Six blocks are used; the committed files in `options/` are a working barrel setup.
